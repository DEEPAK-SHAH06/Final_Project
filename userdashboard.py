...
def fetch_approved_hotels():
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT hotel_id, name, location FROM hotels WHERE is_approved = 1")
    hotels = cur.fetchall()
    conn.close()
    return hotels
...
def fetch_available_rooms(hotel_id):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT room_id, type, price FROM rooms WHERE hotel_id = ? AND is_available = 1", (hotel_id,))
    rooms = cur.fetchall()
    conn.close()
    return rooms
...
def show_rooms_window(user_id, hotel_id):
    ...
    tree = ttk.Treeview(room_win, columns=(
        'Room ID', 'Type', 'Price'), show='headings')
    ...
    def on_book():
        ...
        if selected:
            ...
            book_room(user_id, room_id, check_in_entry.get(),
                      check_out_entry.get())
            room_win.destroy()
        else:
            messagebox.showwarning(
                "Select Room", "Please select a room first.")
...
def show_user_dashboard(user):
    ...
    tk.Label(dash, text=f"Welcome {user[1]}!", font=(
        "Arial", 16, "bold"), bg="#f5f5f5").pack(pady=10)
    tk.Label(dash, text="Search Hotel by Name or Location",
             bg="#f5f5f5", font=("Arial", 11)).pack()

    search_entry = tk.Entry(dash, width=40)
    search_entry.pack(pady=5)

    tree = ttk.Treeview(dash, columns=(
        'Hotel ID', 'Name', 'Location'), show='headings')
    ...
    tk.Button(dash, text="Search", command=on_search,
              width=20, bg="#2196F3", fg="white").pack(pady=2)
    tk.Button(dash, text="View Rooms", command=on_view_rooms,
              width=20, bg="#4CAF50", fg="white").pack(pady=2)
    tk.Button(dash, text="Exit", command=on_exit, width=20,
