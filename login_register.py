...
def show_login_register():
    ...
    def create_labeled_entry(row, label_text, entry_var=None, show=None):
        tk.Label(frame, text=label_text, font=("Arial", 11), bg="#f7f9fc",
                 fg="#333").grid(row=row, column=0, pady=8, sticky='e')
        entry = tk.Entry(frame, font=("Arial", 11), width=25,
                         textvariable=entry_var, show=show)
    ...
    tk.Label(frame, text="Role", font=("Arial", 11), bg="#f7f9fc",
             fg="#333").grid(row=3, column=0, pady=8, sticky='e')
    role_var = tk.StringVar(value="user")
    tk.OptionMenu(frame, role_var, "user", "owner", "admin").grid(
    ...
    tk.Button(button_frame, text="Register", command=on_register, font=("Arial", 11),
              bg="#28a745", fg="white", width=14).pack(side="left", padx=10)

    tk.Button(button_frame, text="Login", command=on_login, font=("Arial", 11),
              bg="#007bff", fg="white", width=14).pack(side="right", padx=10)
